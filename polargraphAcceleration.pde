...
final float MOTOR_MAX_ACCELERATION = 1800.0;  // steps-per-second
...
class Machine {
  ...
  // get maximum acceleration in direction T(x,y) at point actual(x,y)
...
void setup() {
  ...
  pid1 = new PID();
  pid2 = new PID();
  pid1.Kp = pid2.Kp = 50.0;
  pid1.Ki = pid2.Ki = 20.0;
  pid1.Kd = pid2.Kd = 0.0;
  ...
  hs1.setPos(pid1.Kp/50.0);
  hs2.setPos(pid1.Ki/50.0);
  hs3.setPos(pid1.Kd);
...
void step(float dt) {
  ...
  if(d>0) {
    ...
    case 1:{
        ...
        float lvn=dot(dx,dy,-actual.R1x,-actual.R1y)*d;
        float rvn=dot(dx,dy,-actual.R2x,-actual.R2y)*d;
        pid1.Kp = pid2.Kp = 1.0;
        pid1.Ki = pid2.Ki = 0.0;
        pid1.Kd = pid2.Kd = 0.0;
        la = pid1.update(dt,lvn); 
        ra = pid2.update(dt,rvn); 
      } break;
    case 2:{
    ...
    case 4:{
        ...
        la = pid1.update(dt,lan); 
        ra = pid2.update(dt,ran);
        
        float nax = la*actual.R1x + ra * actual.R2x;
        float nay = la*actual.R1y + ra * actual.R2y;
      ...
      case 5: {
        ...
        actual.y +=dy*100;
        actual.IK();
        float lvn=actual.l-ol;
        float rvn=actual.r-or;
        actual.x =ox;
        actual.y =oy;
        actual.IK();
    ...
    float scale=1;
    if(abs(la)>MOTOR_MAX_ACCELERATION) {
      scale = MOTOR_MAX_ACCELERATION/abs(la); 
    }
    if(abs(ra)>MOTOR_MAX_ACCELERATION) {
      scale = MOTOR_MAX_ACCELERATION/abs(ra);
    }
    la *= scale;
    ra *= scale;
